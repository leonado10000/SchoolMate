...
def class_list(request):
    ...
    for batch in batches:
        batch.number_of_students = Student.objects.filter(batch=batch).count()
...
def student_create(request):
    ...
    last_id = Student.objects.annotate(
        num_id=Cast(Substr('student_id', 3), IntegerField())
    ).aggregate(Max('num_id'))['num_id__max']

    form_data = {"student_id": f"s_{(last_id or 0) + 1}"}
    print(form_data)
    # student = Student()
    ...
    if request.method == "POST":
        ...
        try:
            ...
            student.save()
            return student_detail(request, student_id=student.pk)
